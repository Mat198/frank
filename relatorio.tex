...
\iffalse Alteração Matheus
Este relatório apresenta as atividades realizadas no estudo de modelagem e cinemática de robôs, abordando métodos clássicos e modernos aplicados à manipulação robótica e ao uso de técnicas de aprendizagem de máquina. De forma mais específica, os objetivos principais foram:
\fi
...
\begin{itemize}
  \item Descrever robôs utilizando Denavit–Hartenberg e teoria helicoidal.
  \item Estudar e aplicar cinemática direta e inversa.
...
O método de Denavit--Hartenberg (DH) é uma sistematização amplamente utilizada para descrever a geometria de manipuladores robóticos por meio de uma sequência ordenada de transformações homogêneas. Seu objetivo é padronizar a forma de posicionar e orientar sistemas de coordenadas ao longo da cadeia cinemática, permitindo a formulação clara e repetitiva das matrizes de transformação entre elos consecutivos. A partir dessa convenção, a obtenção da cinemática direta torna-se estruturada, reduzindo ambiguidades na modelagem e facilitando a implementação em algoritmos computacionais.

O procedimento consiste em associar um referencial a cada junta e definir quatro parâmetros que descrevem de maneira mínima a transformação entre dois elos consecutivos. Esses parâmetros, conhecidos como $a_i$, $\alpha_i$, $d_i$ e $\theta_i$, caracterizam as operações de rotação e translação necessárias para ir do frame $i$ ao frame $i+1$. O método estabelece regras específicas para posicionar os eixos $z_i$ e $x_i$, garantindo consistência ao longo de toda a cadeia.
...
Portanto, a convenção DH proporciona um procedimento uniforme para construir a cinemática direta do manipulador: multiplica-se sequencialmente as transformações ${}^{0}T_1$, ${}^{1}T_2$, \ldots, ${}^{n-1}T_n$ até obter a matriz final ${}^{0}T_n$ que relaciona a base ao end-effector.

\subsection{Cinemática Direta}

A cinemática direta trata do problema de determinar a posição e a orientação da extremidade operacional (end-effector) de um manipulador robótico a partir das variáveis das juntas. Esse problema é fundamental porque constitui a base de qualquer tarefa de manipulação: antes de planejar trajetórias, controlar forças ou interagir com objetos, é necessário compreender como o movimento interno do robô se traduz em movimento externo do end-effector. Além disso, a cinemática direta fornece a representação geométrica que pode ser usada pela cinemática inversa, Jacobianos e pela modelagem dinâmica.
...
\]
onde $J^\dagger$ é a pseudoinversa da Jacobiana. Esse método, conhecido como cinemática inversa diferencial, converge para uma solução desde que a pose desejada esteja próxima do espaço alcançável e que o sistema não esteja em uma singularidade. 
