...
O cálculo da Jacobiana no Código \ref{jacob_frank0} é realizado construindo-se primeiramente as matrizes de transformação homogênea de cada elo utilizando a biblioteca \textit{SymPy}, definidas a partir dos parâmetros das juntas.
...
Então, essas transformações são multiplicadas para obter a pose final do end-effector e também para determinar as posições da origem de cada sistema de coordenadas de cada junta no espaço, $O_1, O_2, O_3, O_4, O_5$.
...
A Figura \ref{frank_rtb} mostra os eixos de cada junta em relação ao sistema de coordenadas da base:
...
\begin{lstlisting}[caption={Cinemática inversa diferencial}, label={diff_ik}]
import numpy as np
import roboticstoolbox as rtb
...
Além disso, foi realizada a cinemática direta utilizando a Screw Theory, proporcionando uma abordagem alternativa à formulação clássica baseada em DH. Para isso, primeiramente o robô foi movido para a posição em que todas as juntas são 0 conforme a figura \ref{zero_position}. Nessa posição foram definidos os sistemas de coordenadas da base e do ferramenta além dos eixos de screw $s_n$.

\begin{figure}[H]
\centering
    \caption{\footnotesize{FRANK na posição zero com eixos de screw.}}
    \includegraphics[width=1.0\textwidth]{imagens/frank_screw.png}
    \label{zero_position}
    \par       
    \footnotesize{Fonte: elaboração dos autores.}
\end{figure}
...
Em seguida foram calculadas a posição de pontos arbitrários $a_n$ no eixo das juntas na posição zero. Foram escolhidos os pontos definidos como o centro do sistema de coordenadas utilizados na notação de Denavit Hartenberg por conveniencia.
...
\end{bmatrix}, \
a_2 = 
\begin{bmatrix} 0.287 \\ 0 \\ 0.155 \\
\end{bmatrix},\ 
a_3 = 
\begin{bmatrix} 0.287 \\ 0 \\ 0.955 \\
\end{bmatrix}, \
a_4 = 
\begin{bmatrix} 0.287 \\ 0 \\ 1.755 \\
\end{bmatrix}, \
a_5 = 
\begin{bmatrix} 0.287 \\ 0 \\ 2.072 \\
\end{bmatrix}. \
\]
...
\end{bmatrix}, \
s_{v2} = 
\begin{bmatrix} 0.155 \\ 0 \\ -0.287 \\
\end{bmatrix}, \
s_{v3} = 
\begin{bmatrix} 0.955 \\ 0 \\ -0.287 \\
\end{bmatrix}, \
s_{v4} = 
\begin{bmatrix} 1.755 \\ 0 \\ -0.287 \\
\end{bmatrix}, \
s_{v5} = 
\begin{bmatrix} 0 \\ -0.287 \\ 0 \\
\end{bmatrix}. \
\]
...
\[
s_{vn} =
\begin{bmatrix} s_{\omega n} \\  s_{vn} \\
\end{bmatrix}. \
\]

A tranformação é dado então por:

\[
T(\theta_1, \theta_2, \theta_3, \theta_4, \theta_5) = e^{[s_1] \theta_1} e^{[s_2] \theta_2} e^{[s_3] \theta_3} e^{[s_4] \theta_4} e^{[s_5] \theta_5} 
\]

Para efetuar essa operação é preciso converter o vetor 6D para uma matriz de transforamação homogênea. A porção rotacional da matriz é dada pela matriz antissimétrica da porção angular do screw e a tranlação é dada pela porção linear no screw conforme inplementado na função '$twist\_to\_matrix$'.

O Código \ref{screw_ik} abaixo fornece a implementação completa seguindo os passos descritos anteriormente.
...
A transformação obtida T é identica ao resultado utilizando a Robotics Toolbox para os ângulos de (0, -$\pi / 4$, -$\pi/4$, 0 , 0) que correspondem a posição inicial do FRANK.
