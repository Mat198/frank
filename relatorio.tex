...
\subsection{Jacobiano, Cinemática Diferencial e Controle}
...
\subsubsection{Controle Cinemático Baseado no Jacobiano}

% TODO: Corrigir essa seção
Seguindo uma ideia similar ao que foi desenvolvido na cinemática diferencial, é possível realizar um controle de posição das juntas utilizando o Jacobiano.
Dado um erro cartesiano atual
\[
\mathbf{e} = \mathbf{x_d} - \mathbf{x}(\mathbf{q}),
\]
podemos corrigi-lo iterativamente utilizando a atualização
\[
\mathbf{q}_{k+1} = \mathbf{q}_k + J^\dagger(\mathbf{q}_k) \, \mathbf{e} \ dt,
\]
